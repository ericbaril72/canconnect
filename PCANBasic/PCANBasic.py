...
class PCANBasic:
    """
      PCAN-Basic API class implementation
    """      
    def __init__(self,canFOX=False):
        ...
        self.my_handle = None
        if canFOX==False:
            print("Loading PCANBasic dll")
            if platform.system() == 'Windows':
                print("App dirname:",os.path.dirname(__file__))
                try:
                    self.__m_dllBasic = windll.LoadLibrary(os.path.dirname(__file__)+"../PCANBasic.dll")
                except:
                    print("not WIN32")
                try:
                    self.__m_dllBasic = windll.LoadLibrary(os.path.dirname(__file__)+".\\x64\\PCANBasic.dll")
                    print("Loaded as win64")
                except:
                    print("not running x32 python")
                    exit()
                #self.__m_dllBasic = windll.LoadLibrary(os.path.dirname(__file__)+"/PCANBasic.dll")
            else:
    ...
    def Detect(self):
        ...
        try:
            ...
            print("not second choice")

        rez=self.__m_dllBasic.CAN_Uninitialize(PCAN_NONEBUS)
        print("unintit all:",rez)
        print("****************")
        #time.sleep(1)
        for i in range(0x51, 0x59):  # TPCANHandle List for PCAN_USB
            #PCAN_CHANNEL_CONDITION

            rez = self.Initialize(TPCANHandle(i), PCAN_BAUD_250K)
            time.sleep(0.1)
            rez = self.GetStatus(TPCANHandle(i))
            print("IF Status:",hex(rez))
            status = rez
            if rez==0x8:
                rez2=self.Reset(TPCANHandle(i))
                print("Reset:",rez2)
                rez = self.GetStatus(TPCANHandle(i))
                print("IF Status:",hex(rez))
                rez = self.Initialize(TPCANHandle(i), PCAN_BAUD_250K, PCAN_NONE, 0x100, 3)
                print("init status:",hex(rez))
            if rez == PCAN_ERROR_OK:


                hasPeak.append(i)
                print("Found PCAN:           {}\t{}\t{}".format(hex(i), rez, hex(status)))

            elif rez!=PCAN_ERROR_ILLHW:
                pass
                print("error",hex(rez))
                rez1= self.GetValue(TPCANHandle(i),PCAN_CHANNEL_CONDITION)
                print("ChannelCondition:",i,rez1)
                if rez1[1]==2:
                    rez3=self.Reset(TPCANHandle(i))
                    print("Reset:",hex(rez3))
                    rez1= self.GetValue(TPCANHandle(i),PCAN_CHANNEL_CONDITION)
                    print("ChannelCondition:",i,rez1)
                    rez = self.Initialize(TPCANHandle(i), PCAN_BAUD_250K, PCAN_NONE, 0x100, 3)
                    print("init status:",hex(rez))
                ...
                #print("unintit",hex(rez))

                #self.__m_dllBasic.CAN_Uninitialize(TPCANHandle(i))
                #print("no PCAN @ ",hex(i),hex(rez))
            print()
        rez=self.__m_dllBasic.CAN_Uninitialize(PCAN_NONEBUS)
        print("unintit all",rez)
        #rez=self.__m_dllBasic.CAN_Uninitialize(PCAN_NONEBUS)
        ...
        try:
            ...
            print("No canfix"
                  "")
            return {"Peak":hasPeak,"CanFox":hasCanFox,"Error":"Error win win64 canfox DLL"}
        ...
        cntval = c_int(0)
        rez2 = func(cntval)
        hasCanFox = cntval.value




        # back to last configured
        self.__m_dllBasic = tempdll
        return {"Peak":hasPeak,"CanFox":hasCanFox}
    ...
    def InitCanfox(
        ...
        if totalDevice > 0:
            list = (canDEVLIST * totalDevice)()
            func1 = self.__m_dllBasic.canGetDeviceList
            func1.argtypes = [POINTER(canDEVLIST)]
            ...
            # Connect to device0
            func_canOpen = self.__m_dllBasic.canOpen
            net_num=c_long(105)
            func_canOpen.argtypes = [c_long, c_long, c_long, c_long, c_long, c_char_p, c_char_p, c_char_p, POINTER(c_void_p)]
            func_canOpen.restype = c_long
            ...
            t_stat = c_int(4)
            rezC = func_canOpen(c_long(105), 0, 0, 100, 100, b"", b"", b"", byref(my_handle))
            print("zC:",rezC)
            self.my_handle = my_handle
            ...
            while cnt<0:
                ...
                canMsg = canMSG()
                len = c_int(1)
                res = self.__m_dllBasic.canRead(my_handle, byref(canMsg), byref(len))

                if res==0:
                    dataString = "\t"
                    for i in range(0, int(canMsg.len)):
                        dataString += "{:02X} ".format(canMsg.data[i])
                    x = (canMsg.timestamp / 10000)
                    print(time.strftime('%H:%M:%S', time.gmtime(x)), hex(canMsg.id),dataString)
                    res = TPCANStatus(0x0000)
    ...
    def canClose(self):
        rezg = self.__m_dllBasic.canClose(self.my_handle)
        print("Closed", rezg)
        return TPCANStatus(rezg)
    ...
    def Initialize(
        ...
        """
        if self.canFOX:
            return self.InitCanfox(  self.my_handle,         Btr0Btr1, HwType = TPCANType(0),  IOPort = c_uint(0),Interrupt = c_ushort(0))
        try:
    ...
    def Read(
        ...
        try:
            ...
            if self.canFOX:
                ...
                canMsg = canMSG()
                len = c_int(1)
                res = self.__m_dllBasic.canReadNoWait(self.my_handle, byref(canMsg), byref(len))

                if res>=0:
                    #print("res:",res,len,len.value,hex(canMsg.id))
                    res=TPCANStatus(0x0000)
                    msg.ID = canMsg.id
                    msg.LEN = canMsg.len
                    msg.DATA = canMsg.data
                    memmove(canMsg.data, byref(msg.DATA), msg.LEN)
    ...
    def Write(
        ...
        try:
            if self.canFOX:
                ...
                func_canConfirmedTransmit = self.__m_dllBasic.canConfirmedTransmit
                net_num=c_long(105)
                func_canConfirmedTransmit.argtypes = [c_void_p, c_void_p,c_void_p]
                func_canConfirmedTransmit.restype = c_long

                func_canConfirmedTransmit.restype = c_long

                res=func_canConfirmedTransmit(my_handle ,byref(canMsg),byref(len))
                print("Send:",res,len)
                return TPCANStatus(res)
...
if __name__ == '__main__':
    ...
    canRead = PCANBasic(canFOX=True)
    #ifstatus = canRead.Initialize(0, 0, 0, 0x100, 3)
    canMsg = canMSG()
    canMsg.len = 8
    canMsg.id = 0x33
    canMsg.data[0]=0x11

    res=canRead.Write(0,canMsg)
